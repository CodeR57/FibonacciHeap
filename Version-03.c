...
struct heap* Fib_Heap_Union (struct heap* H1, struct heap* H2){ //O(1)
	...
	if (H1->min&&H2->min){
		H->min = H1->min;
		H->min->left->right = H2->min->right; //4 строки - объединение списков
...
struct node* Fib_Heap_Extract_Min (struct heap* H){ //O(log n)
	struct node* z = H->min;
	if (z) {
		if (z == z->right) { // z - единственный в списке корней
				H->min = z->child; //помечаем указатель на ноль или ребенка
				Consolidate(H);
		} else {
			if (z->child == NULL){
				z->left->right = z->right; //удаляем z из списка корней
				z->right->left = z->left;
				H->min = z->right;
				Consolidate(H);
			} else {
				z->left->right = z->child; //удаляем z из списка корней
	...
	for (i=0; i<=Dn; i++){
		if (A[i]){
			if ((H->min == NULL) || (A[i]->key < H->min->key)) {
				H->min = A[i];
				//H->min->left->right = A[i]; //кажется, и без этого работает
				//A[i]->left = H->min->left;
				//A[i]->right = H->min;
				//H->min->left = A[i];
			}
